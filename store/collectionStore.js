...
const CACHE_PREFIX = "tres-deseos:colecciones:v1:";
const MAX_AGE_MS = 5 * 60 * 1000;
const VERSION_DEPENDENCIES = ["Colecciones", "Productos", "Categorias", "Imagenes"];
...
function buildDetailUrl(slug, filters = {}) {
  const url = new URL(endpoints.coleccionDetalle(slug));
...
async function fetchAndCache(url) {
  ...
  if (!data) {
    throw new Error("La API no devolvió información de colecciones.");
  }
...
export const collectionStore = {
  loadAll(options = {}) {
    return load(endpoints.colecciones, options);
  },
